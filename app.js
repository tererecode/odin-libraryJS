...
const newBookBtn = document
  ...
  .addEventListener("click", Library.popUpForm);
const closeBtn = document
  .getElementById("close_popup_btn")
  .addEventListener("click", Library.popUpForm);
...
class Book {
  ...
  addBookToContainer() {
    const bookCardHtml =
      Book.createBookCard(`<div class="book__card" data-index="${myLibrary.libArray.indexOf(
        this
      )}">
            <h1>${this.title}</h1>
            <h2>${this.author}</h2>
    ...
    let cardElement = libraryContainer.querySelector(
      `[data-index="${myLibrary.libArray.indexOf(this)}"]`
    );
    cardElement
      .querySelector(".change-read-btn")
      .addEventListener("click", (e) =>
        myLibrary.libArray[e.target.parentNode.dataset.index].changeRead()
      );
    cardElement
      .querySelector(".delete-btn")
      .addEventListener("click", (e) =>
        myLibrary.libArray[e.target.parentNode.dataset.index].removeBook()
      );
  }
